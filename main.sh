...
#### Cam checks ####
	...
	elif [[ $CHECK =~ "Netcam" ]]; then
		DATA=`curl -sL -w "%{http_code}\\n" "http://$1/anony/mjpg.cgi" -o /dev/null` #Trendnet Remote Security Bypass
		if [[ $DATA =~ "200" ]]; then
			echo -e "\e[1;49;93m_-_-_-\e[0;49;96m $1 + Trendnet-CAM \e[1;49;93m-_-_-_\e[0m"
		fi
	fi
elif [[ $2 == '2' ]]; then
#### File grab check ####
	if [[ $CHECK =~ "Easy File Sharing" ]]; then 
...
#### 401 check ####
	...
	if [[ $CHECK =~ "401 Unauthorized" ]]; then
